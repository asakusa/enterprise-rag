...
"""
企业文档检索 RAG 系统
基于 Amazon Bedrock Knowledge Base 实现
"""
...
class EnterpriseRAG:
    ...
    def upload_documents(self, documents_path, bucket_name):
        """上传企业文档到S3"""
        logger.info(f"开始上传文档从 {documents_path} 到 {bucket_name}")
        
        for root, dirs, files in os.walk(documents_path):
            for file in files:
                if file.endswith(('.md', '.txt', '.pdf', '.docx')):
                    file_path = os.path.join(root, file)
                    s3_key = f"documents/{file}"
                    
                    logger.info(f"上传文件: {file_path} -> s3://{bucket_name}/{s3_key}")
    ...
    def create_knowledge_base(self):
        ...
        self.kb_id, self.ds_id = self.kb_helper.create_or_retrieve_knowledge_base(
            ...
            data_bucket_name=None  # 让系统自动创建bucket
        )
        
        bucket_name = self.kb_helper.data_bucket_name
        logger.info(f"知识库创建成功: KB ID={self.kb_id}, DS ID={self.ds_id}, Bucket={bucket_name}")
    ...
    def create_rag_agent(self):
        ...
        agent = Agent.create(
            ...
            instructions="""
            你是一个专业的企业文档检索助手，具有以下能力：
            
            1. **文档检索**：能够准确检索企业内部文档，包括：
               - 公司政策和制度
               - IT支持和技术文档
               - 财务管理制度
               - 人事管理规定
               
            2. **信息提取**：从检索到的文档中提取关键信息，提供准确、简洁的答案
            ...
            请始终基于企业知识库中的信息回答问题，不要编造或猜测信息。
            """,
            kb_id=self.kb_id,
            kb_descr="企业内部文档知识库，包含公司各项政策制度和操作指南",
    ...
    def setup_complete_system(self, documents_path):
        ...
        # 1. 创建知识库
        bucket_name = self.create_knowledge_base()
        
        # 2. 上传文档
        self.upload_documents(documents_path, bucket_name)
        
        # 3. 等待文档上传完成
        logger.info("等待文档上传完成...")
        time.sleep(10)
    ...
    def cleanup(self):
        ...
        if self.kb_id:
            try:
                self.kb_helper.delete_kb(self.kb_name, delete_s3_bucket=True)
                logger.info("知识库删除成功")
            except Exception as e:
